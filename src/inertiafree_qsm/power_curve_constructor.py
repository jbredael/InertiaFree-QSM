...
This module provides functionality to generate optimized power curves using
independent per-wind-speed optimization. It includes constraint handling and
...
3. For each wind speed, call ``CycleOptimizer`` to optimize the cycle.
...
class PowerCurveConstructor:
    """Generate optimized power curves for an AWE system.

    Full power curves are generated by optimizing each wind speed independently.
    Single wind speed simulations can also be run directly from the configured
    cycle settings.
    ...
    def _generate_wind_speed_array(self):
        """Generate wind speed array from optimization settings.
        ...
        """
        settings = self.simulation_settings['optimization']
        ...
        if vw_cut_in is None:
            raise ValueError("cut_in wind speed must be specified in optimization settings")
        if vw_cut_out is None:
            raise ValueError("cut_out wind speed must be specified in optimization settings")
    ...
    def _resolve_wind_speeds(self, wind_speeds, verbose):
        ...
        if wind_speeds is not None:
            ...
            return wind_speeds
        return self._generate_wind_speed_array()
    ...
    def generate_power_curves(
        ...
        """Generate optimized power curves using SLSQP.

        Each wind speed is solved independently from the configured optimizer
        starting point.
        ...
        Args:
            ...
            profile_ids (list, optional): List of profile IDs (1-indexed) to calculate.
                If None, calculates all profiles.
        ...
        wind_speeds = self._resolve_wind_speeds(wind_speeds, verbose)
    ...
    def _extract_time_history(self, time_list, kinematics, steady_states,
        ...
        Direct and optimized single-cycle paths both provide lists of kinematics
        and steady-state objects with the same attributes.
