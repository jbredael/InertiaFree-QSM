...
class PhaseError(Exception):
    ...
    Error codes:
        ...
        2: Reeling in during reel-out phase (traction phase).
...
class SystemProperties:
    ...
    Attributes:
        ...
        max_generator_power (float): Maximum generator power [W]
...
class SteadyState:
    ...
    def find_state(self, system_properties, environment_state, basic_kinematics, print_details=False):
        ...
        # Iterative procedure to determine the kinematic ratio.
        ...
        if self.impose_operational_limits:
            if isinstance(self, TractionPhase):          
                ...
                if max_power is not None and new_state.power_ground > max_power:
                    self._set_limited_reeling_speed(
                        new_state, self.steady_state_config, sys_props, env_state,
                        kinematics, max_power, max_force, max_speed,
                    )
...
class TractionPhase(Phase):
    ...
    def _set_limited_reeling_speed(
            self, state, steady_state_config, sys_props, env_state, kinematics,
            max_power, max_force, max_speed):
        """Set traction reel-out speed to the generator power cap."""
        v_violating = state.reeling_speed
        v_limited = max_power / max_force
        if max_speed is not None:
            v_limited = min(v_limited, max_speed)

        state.control_settings = ('reeling_speed', v_limited)
        state.find_state(sys_props, env_state, kinematics)

        if (state.power_ground < max_power
                and (max_speed is None or v_limited < max_speed)
                and v_violating > v_limited):
            def _power_residual(v):
                s = SteadyState(steady_state_config)
                s.control_settings = ('reeling_speed', v)
                s.find_state(sys_props, env_state, kinematics)
                return s.power_ground - max_power

            try:
                v_limited = brentq(_power_residual, v_limited, v_violating, xtol=1e-3)
                if max_speed is not None:
                    v_limited = min(v_limited, max_speed)
                state.control_settings = ('reeling_speed', v_limited)
                state.find_state(sys_props, env_state, kinematics)
    ...
    def get_elevation_angle(self, system_properties, environment_state,
        ...
        try:
            ...
            if max_power is not None and probe_ss.power_ground > max_power:
                self._set_limited_reeling_speed(
                    probe_ss, steady_state_config, system_properties,
                    environment_state, probe_kin, max_power, max_force, max_speed,
                )
...
class Cycle(TimeSeries):
    ...
    @classmethod
    def calculate_phase_electrical_energy(cls, phase, system_properties):
        ...
        electrical_power = cls.calculate_electrical_power(mechanical_power, system_properties)
        return float(np.trapezoid(electrical_power, phase.time))

    def build_cycle_energy_power_kpis(self, system_properties):
        """Build mechanical and electrical energy/average-power KPI dictionaries."""
        phases = {
            'out': getattr(self, 'traction_phase', None),
            'trans_rori': getattr(self, 'transition_rori_phase', None),
            'in': getattr(self, 'retraction_phase', None),
            'trans_riro': getattr(self, 'transition_riro_phase', None),
        }
        ...
        mechanical_energy['cycle'] = float(getattr(self, 'energy', 0.0) or 0.0)

        electrical_energy = {
            name: self.calculate_phase_electrical_energy(phase, system_properties)
            for name, phase in phases.items()
        }
        electrical_energy['cycle'] = electrical_energy['out'] + electrical_energy['in']
        if getattr(self, 'include_transition_energy', False):
            electrical_energy['cycle'] += (
                electrical_energy['trans_riro'] + electrical_energy['trans_rori']
            )

        duration = {
            'cycle': float(getattr(self, 'duration', 0.0) or 0.0),
            **{
        ...
        for name in ('cycle', 'in', 'trans_riro', 'trans_rori', 'out'):
            ...
            if name == 'cycle':
                mechanical_average_power[name] = float(getattr(self, 'average_power', 0.0) or 0.0)
            else:
